...
{!! Form::open(['url' => 'transaksi','name' => 'formtrans','id'=>'formtrans', 'onSubmit' => 'return ConfirmSimpanTransaksi()']) !!}
...
<div class="modal-dialog modal-lg">
	...
	<div class="modal-content">
		...
		<tbody>
			...
			?>
			{!! Form::hidden('id_users', $iduser, ['id'=>'id_users']) !!}
...
formtrans.addEventListener('submit', handleSubmit);
var submitTimer;
...
function ConfirmSimpanTransaksi(){
	if(panjang > 0){
		var iduser = parseInt(document.getElementById("id_users").value);
		var xmlhttpget = new XMLHttpRequest();
		var urlget = "../keranjangjson/"+jenis+"/iduser/"+iduser;
		xmlhttpget.open("GET", urlget, true);
		xmlhttpget.send();
		xmlhttpget.onreadystatechange=function() {
		if (xmlhttpget.readyState == 4 && xmlhttpget.status == 200) {
		    var keranjang = xmlhttpget.responseText;
		    updatestokserver(keranjang);
		    }
		}
	}
}
function updatestokserver(keranjang){
	console.log(keranjang);
	var xmlhttp = new XMLHttpRequest();
	var url = "http://arieskuspanwibowo.com/transaksisinkron";
		xmlhttp.open("POST", url, true);
		xmlhttp.setRequestHeader('x-csrf-token', csrfToken);
		xmlhttp.setRequestHeader("Content-type","application/json;charset=UTF-8");
		xmlhttp.send(keranjang);
